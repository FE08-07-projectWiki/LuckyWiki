...
export default function WikiEditForm() {
  ...
  const editUser: editUserType = async (code, data) => {
    await patchUser(code, data);
  };

  const userCheck = userId !== undefined && user?.profile && user?.profile.id !== userId;
  ...
  return (
    <section className={styles.editContiner}>
      {isModalOpen && (
        <ModalComponent isOpen={isModalOpen} title="주의" onClose={handleCloseModal}>
          본인이 아닌 프로필은 수정이 불가합니다.
        </ModalComponent>
      )}
      {isSubmitModalOpen && (
        <ModalComponent isOpen={isSubmitModalOpen} onClose={handleCloseSubmitModal}>
          수정되었습니다.
        </ModalComponent>
      )}
